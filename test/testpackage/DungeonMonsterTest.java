...
/**
 * Class to test the dungeon with monsters in them.
 */
public class DungeonMonsterTest {
  ...
  public void setUp() {
    ...
    dungeon = new Dungeon(6, 8, false,0, 0.3,
            4,2, testRandomNumberGenerator);
  ...
  public void checkSmellAtOneLocationBeforeEnd() {
    dungeon.getPlayer().setCurrentLocation(dungeon.getPlayer()
            .getFinalLocation().getDirectionOfLocation(Direction.NORTH));
    Smell smell = dungeon.getPlayer().getCurrentLocation().checkSmellStrength();
  ...
  public void checkSmellAtTwoLocationsBeforeEnd() {
    dungeon.getPlayer().setCurrentLocation(dungeon.getLocations()[3][7]);
    Smell smell = dungeon.getPlayer().getCurrentLocation().checkSmellStrength();
  ...
  @Test
  public void testNoOfPits() {
    int noOfPits = 0;
    for (int rowPos = 0; rowPos < dungeon.getRows(); rowPos++) {
      for (int colPos = 0; colPos < dungeon.getCols(); colPos++) {
        if (dungeon.getLocations()[rowPos][colPos].isPit()) {
          noOfPits++;
        }
      }
    }
    assertEquals(noOfPits, dungeon.getNoOfPits());
  }

  @Test
  public void testGameOverWhenPlayerEntersPit() {
    assertTrue(dungeon.getLocations()[5][7].isPit());
    dungeon.getPlayer().setCurrentLocation(dungeon.getLocations()[4][7]);
    dungeon.movePlayer(Direction.SOUTH);
    assertFalse(dungeon.getPlayer().isAlive());
  }
}
